...
public class ThreePointAligment : MonoBehaviour
{
    ...
    public Accord.Math.Vector3 scaleFactors;
    ...
    void Start()
    {
        ...
        {
            points_1.Add(new Accord.Math.Vector3(
                pointCloud_1.transform.GetChild(i).position.x*scaleFactors.X,
                pointCloud_1.transform.GetChild(i).position.y*scaleFactors.X,
                pointCloud_1.transform.GetChild(i).position.z*scaleFactors.X));
        }
    ...
    void changeColor(Accord.Math.Vector3 p){

        for(int i = 0; i < pointCloud_1.transform.childCount; i++)
        {

            if (pointCloud_1.transform.GetChild(i).position.x == p.X && pointCloud_1.transform.GetChild(i).position.y == p.Y && pointCloud_1.transform.GetChild(i).position.z == p.Z)
            {
                var renderer = pointCloud_1.transform.GetChild(i).GetComponent<Renderer>();
    ...
    void GetBestTransformation(List<Accord.Math.Vector3> points_1, List<Accord.Math.Vector3> points_2, out Accord.Math.Vector3 translation, out Accord.Math.Matrix3x3 rotation, out float scale){
        ...
        for(int i = 0; i < iteration ; i++){
            ...
            if(bestInliers > points_1.Count / 2) break; //Meaning %50 match has been gotten!
        }
